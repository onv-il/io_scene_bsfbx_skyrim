...
def elem_props_get_integer(elem, elem_prop_id, default=None):
    ...
    if elem_prop is not None:
        ...
        # we could allow other number types
        assert (elem_prop.props_type[4] in {
                ...
                if fbx_prop.props[0] == b'UDP3DSMAX':
                    ...
                    for item in items.split('\r\n'):
                        if item:
                            ...
                            if len(split_item) != 2:
                                print(
                                    "cannot parse UDP3DSMAX custom property '%s', ignoring..." %
                                    item)
                            else:
                                prop_name, prop_value = split_item
                                prop_name = validate_blend_names(
                                    prop_name.strip().encode('utf-8'))
                    ...
                    if prop_type in {b'Vector', b'Vector3D', b'Color', b'ColorRGB'}:
                        assert (fbx_prop.props_type[4:7] == bytes(
                            (data_types.FLOAT64,)) * 3)
                        blen_obj[prop_name] = fbx_prop.props[4:7]
                    elif prop_type in {b'Vector4', b'ColorRGBA'}:
                        assert (fbx_prop.props_type[4:8] == bytes(
                            (data_types.FLOAT64,)) * 4)
                        blen_obj[prop_name] = fbx_prop.props[4:8]
                    elif prop_type == b'Vector2D':
                        assert (fbx_prop.props_type[4:6] == bytes(
                            (data_types.FLOAT64,)) * 2)
                        ...
                        assert (fbx_prop.props_type[4] == data_types.STRING)
                        blen_obj[prop_name] = fbx_prop.props[4].decode(
                            'utf-8', 'replace')
                        ...
                        print("WARNING: User property type '%s' is not supported" %
...
def blen_read_object_transform_do(transform_data):
    ...
    # rotation
    def to_rot(rot, rot_ord): return Euler(
        convert_deg_to_rad_iter(rot), rot_ord).to_matrix().to_4x4()
    lcl_rot = to_rot(transform_data.rot,
                     transform_data.rot_ord) @ transform_data.rot_alt_mat
...
def blen_read_object_transform_preprocess(fbx_props, fbx_obj, rot_alt_mat, use_prepost_rot):
    ...
    loc = list(elem_props_get_vector_3d(
        fbx_props, b'Lcl Translation', const_vector_zero_3d))
    rot = list(elem_props_get_vector_3d(
        fbx_props, b'Lcl Rotation', const_vector_zero_3d))
    sca = list(elem_props_get_vector_3d(
        fbx_props, b'Lcl Scaling', const_vector_one_3d))

    geom_loc = list(elem_props_get_vector_3d(
        fbx_props, b'GeometricTranslation', const_vector_zero_3d))
    geom_rot = list(elem_props_get_vector_3d(
        fbx_props, b'GeometricRotation', const_vector_zero_3d))
    geom_sca = list(elem_props_get_vector_3d(
        fbx_props, b'GeometricScaling', const_vector_one_3d))

    rot_ofs = elem_props_get_vector_3d(
        fbx_props, b'RotationOffset', const_vector_zero_3d)
    rot_piv = elem_props_get_vector_3d(
        fbx_props, b'RotationPivot', const_vector_zero_3d)
    sca_ofs = elem_props_get_vector_3d(
        fbx_props, b'ScalingOffset', const_vector_zero_3d)
    sca_piv = elem_props_get_vector_3d(
        fbx_props, b'ScalingPivot', const_vector_zero_3d)

    is_rot_act = elem_props_get_bool(fbx_props, b'RotationActive', False)

    if is_rot_act:
        if use_prepost_rot:
            pre_rot = elem_props_get_vector_3d(
                fbx_props, b'PreRotation', const_vector_zero_3d)
            pst_rot = elem_props_get_vector_3d(
                fbx_props, b'PostRotation', const_vector_zero_3d)
        else:
    ...
    return FBXTransformData(
        loc, geom_loc, rot, rot_ofs, rot_piv, pre_rot, pst_rot, rot_ord, rot_alt_mat, geom_rot,
        sca, sca_ofs, sca_piv, geom_sca)
        ...
        for item in curves:
            ...
            if times[idx] >= curr_fbxktime:
                if idx == 0:
                    ...
                    ifac = (curr_fbxktime -
                            times[idx - 1]) / (times[idx] - times[idx - 1])
                    curr_values.append(
                        ((values[idx] - values[idx - 1]) * ifac + values[idx - 1], fbx_curve))
        curr_blenkframe = (curr_fbxktime - fbx_start_offset) * \
            timefac + blen_start_offset
...
def blen_read_animations_action_item(action, item, cnodes, fps, anim_offset):
    ...
    if isinstance(item, Material):
        ...
        if rot_mode == 'QUATERNION':
            props[1] = (bl_obj.path_from_id("rotation_quaternion"),
                        4, grpname or "Quaternion Rotation")
        elif rot_mode == 'AXIS_ANGLE':
            props[1] = (bl_obj.path_from_id("rotation_axis_angle"),
                        4, grpname or "Axis Angle Rotation")
        else:  # Euler
            props[1] = (bl_obj.path_from_id("rotation_euler"),
                        3, grpname or "Euler Rotation")
    ...
    for fc_key, key_values in keyframes.items():
        ...
        linear_enum_value = bpy.types.Keyframe.bl_rna.properties[
    ...
    for as_uuid, ((fbx_asdata, _blen_data), alayers) in stacks.items():
        ...
        for al_uuid, ((fbx_aldata, _blen_data), items) in alayers.items():
            ...
            for item, cnodes in items.items():
                ...
                if action is None:
                    action_name = "|".join(
                        (id_data.name, stack_name, layer_name))
                ...
                blen_read_animations_action_item(
                    action, item, cnodes, scene.render.fps, anim_offset)
...
def blen_read_geom_layerinfo(fbx_layer):
    return (
        validate_blend_names(
            elem_find_first_string_as_bytes(fbx_layer, b'Name')),
        elem_find_first_string_as_bytes(fbx_layer, b'MappingInformationType'),
        elem_find_first_string_as_bytes(
            fbx_layer, b'ReferenceInformationType'),
    )


def blen_read_geom_array_setattr(
        generator, blen_data, blen_attr, fbx_data, stride, item_size, descr, xform):
    ...
    if xform is not None:
        if isinstance(blen_data, list):
            if item_size == 1:
                ...
                def _process(blend_data, blen_attr, fbx_data, xform, item_size, blen_idx, fbx_idx):
                    blen_data[blen_idx] = xform(
                        fbx_data[fbx_idx:fbx_idx + item_size])
        else:
            if item_size == 1:
                def _process(blend_data, blen_attr, fbx_data, xform, item_size, blen_idx, fbx_idx):
                    setattr(blen_data[blen_idx], blen_attr,
                            xform(fbx_data[fbx_idx]))
            else:
                def _process(blend_data, blen_attr, fbx_data, xform, item_size, blen_idx, fbx_idx):
                    setattr(blen_data[blen_idx], blen_attr, xform(
                        fbx_data[fbx_idx:fbx_idx + item_size]))
    else:
        if isinstance(blen_data, list):
            if item_size == 1:
    ...
    for blen_idx, fbx_idx in generator:
        if check_skip(blen_idx, fbx_idx):
            continue
        _process(blen_data, blen_attr, fbx_data,
                 xform, item_size, blen_idx, fbx_idx)
...
def blen_read_geom_array_error_mapping(descr, fbx_layer_mapping, quiet=False):
    if not quiet:
        print("warning layer %r mapping type unsupported: %r" %
              (descr, fbx_layer_mapping))


def blen_read_geom_array_error_ref(descr, fbx_layer_ref, quiet=False):
    if not quiet:
        print("warning layer %r ref type unsupported: %r" %
              (descr, fbx_layer_ref))
    ...
    if fbx_layer_mapping == b'ByVertice':
        if fbx_layer_ref == b'Direct':
            ...
            blen_read_geom_array_setattr(
                blen_read_geom_array_gen_direct(fbx_layer_data, stride),
            ...
            if fbx_layer_index is None:
                blen_read_geom_array_setattr(
                    blen_read_geom_array_gen_direct(
                        fbx_layer_data, stride),
                    blen_data, blen_attr, fbx_layer_data, stride,
                    item_size, descr, xform)
            else:
                blen_read_geom_array_setattr(
                    blen_read_geom_array_gen_indextodirect(
                        fbx_layer_index, stride),
                    blen_data, blen_attr, fbx_layer_data, stride,
                    item_size, descr, xform)
            return True
...
def blen_read_geom_array_mapped_polyloop(
    ...
    if fbx_layer_mapping == b'ByPolygonVertex':
        ...
        if fbx_layer_ref == b'Direct':
            assert (fbx_layer_index is None)
            blen_read_geom_array_setattr(
                blen_read_geom_array_gen_direct_looptovert(
                    mesh, fbx_layer_data, stride),
                blen_data, blen_attr, fbx_layer_data, stride, item_size,
                descr, xform)
            return True
        blen_read_geom_array_error_ref(descr, fbx_layer_ref, quiet)
    elif fbx_layer_mapping == b'AllSame':
...
def blen_read_geom_layer_uv(fbx_obj, mesh):
    for layer_id in (b'LayerElementUV',):
        for fbx_layer in elem_find_iter(fbx_obj, layer_id):
            ...
            fbx_layer_index = elem_prop_first(
                elem_find_first(fbx_layer, b'UVIndex'))
...
def blen_read_geom_layer_color(fbx_obj, mesh):
    ...
    for layer_id in (b'LayerElementColor',):
        for fbx_layer in elem_find_iter(fbx_obj, layer_id):
            ...
            fbx_layer_data = elem_prop_first(
                elem_find_first(fbx_layer, b'Colors'))
            fbx_layer_index = elem_prop_first(
                elem_find_first(fbx_layer, b'ColorIndex'))

            # Always init our new layers with full white opaque color.
            color_lay = mesh.vertex_colors.new(
                name=fbx_layer_name, do_init=False)
...
def blen_read_geom_layer_smooth(fbx_obj, mesh):
    ...
    if fbx_layer_mapping == b'ByEdge':
        ...
        return blen_read_geom_array_mapped_polygon(
            ...
            fbx_layer_mapping, fbx_layer_ref,
            1, 1, layer_id,
            # smoothgroup bitflags, treat as booleans for now
            xform=lambda s: (s != 0),
        )
    else:
        print("warning layer %r mapping type unsupported: %r" %
              (fbx_layer.id, fbx_layer_mapping))
        return False
...
def blen_read_geom_layer_normal(fbx_obj, mesh, xform=None):
    ...
    fbx_layer_data = elem_prop_first(elem_find_first(fbx_layer, layer_id))
    fbx_layer_index = elem_prop_first(
        elem_find_first(fbx_layer, b'NormalsIndex'))
    ...
    for blen_data, blen_data_type, is_fake, func in tries:
        ...
        if func(mesh, bdata, "normal", fbx_layer_data, fbx_layer_index, fbx_layer_mapping,
                fbx_layer_ref, 3, 3, layer_id, xform, True):
...
def blen_read_geom(fbx_tmpl, fbx_obj, settings):
    ...
    geom_mat_no = Matrix(
        settings.global_matrix_inv_transposed) if settings.bake_space_transform else None
    ...
    fbx_polys = elem_prop_first(
        elem_find_first(fbx_obj, b'PolygonVertexIndex'))
    ...
    if geom_mat_co is not None:
        ...
        fbx_verts = array.array(
            fbx_verts.typecode, _vcos_transformed_gen(fbx_verts, geom_mat_co))
...
def blen_read_shape(fbx_tmpl, fbx_sdata, fbx_bcdata, meshes, scene):
    ...
    indices = elem_prop_first(elem_find_first(
        fbx_sdata, b'Indexes'), default=())
    dvcos = tuple(co for co in zip(
        *[iter(elem_prop_first(elem_find_first(fbx_sdata, b'Vertices'), default=()))] * 3))
    # We completely ignore normals here!
    weight = elem_prop_first(elem_find_first(
        fbx_bcdata, b'DeformPercent'), default=100.0) / 100.0
    vgweights = tuple(vgw / 100.0 for vgw in elem_prop_first(
        elem_find_first(fbx_bcdata, b'FullWeights'), default=()))
    ...
    for me, objects in meshes:
        vcos = tuple((idx, me.vertices[idx].co + Vector(dvco))
                     for idx, dvco in zip(indices, dvcos))
        ...
        if create_vg:
            vgoups = add_vgroup_to_objects(
                indices, vgweights, kb.name, objects)
...
def blen_read_material(fbx_tmpl, fbx_obj, settings):
    ...
    # TODO: dont' load or create materials that are never used. this will require a little bit of pre-scanning the FBX to see which materials never get referenced by polygons
    # dig through the fbx_obj data structure and find a TargetMaterialFile property.
    def get_mat_path():
        for elem in fbx_obj.elems:
            for e in elem.elems:
                if e.id == b'P' and 'TargetMaterialFile' in e.props[0].decode('ascii'):
                    mat_path = e.props[4].decode('ascii')
                    if mat_path.startswith('\\Materials\\'):
                        mat_path = mat_path[11:]
                    print(
                        f"Found a BGS Material in the incoming FBX: {mat_path}")
                    return mat_path
        return None

    if settings.use_bgs_materials:
        mat_path = get_mat_path()
        if mat_path:
            # look at every material in bpy.data and see if any match this material path. if so return _that_ material instead of creating a new one.
            for m in bpy.data.materials:
                if m.get("MAT_PATH", False) and m["MAT_PATH"] == mat_path:
                    print(
                        f"Found a matching BGS material that was already loaded, using {m.name}!")
                    return m
    ...
    ma_wrap = node_shader_utils.PrincipledBSDFWrapper(
        ma, is_readonly=False, use_nodes=True)
    ma_wrap.base_color = elem_props_get_color_rgb(
        fbx_props, b'DiffuseColor', const_color_white)
    ...
    ma_wrap.specular = elem_props_get_number(
        fbx_props, b'SpecularFactor', 0.25) * 2.0
        ...
        if alpha is None:
            alpha = 1.0 - \
                elem_props_get_color_rgb(
                    fbx_props, b'TransparentColor', const_color_black)[0]
    ma_wrap.alpha = alpha
    ma_wrap.metallic = elem_props_get_number(
        fbx_props, b'ReflectionFactor', 0.0)
    # We have no metallic (a.k.a. reflection) color...
    # elem_props_get_color_rgb(fbx_props, b'ReflectionColor', const_color_white)
    ma_wrap.normalmap_strength = elem_props_get_number(
        fbx_props, b'BumpFactor', 1.0)
    # Emission strength and color
    ma_wrap.emission_strength = elem_props_get_number(
        fbx_props, b'EmissiveFactor', 1.0)
    ma_wrap.emission_color = elem_props_get_color_rgb(
...
def blen_read_texture_image(fbx_tmpl, fbx_obj, basedir, settings):
    ...
    if not filepath:
        ...
        filepath = filepath.replace(
            '\\', '/') if (os.sep == '/') else filepath.replace('/', '\\')
...
def blen_read_camera(fbx_tmpl, fbx_obj, global_scale):
    ...
    camera.type = 'ORTHO' if elem_props_get_enum(
        fbx_props, b'CameraProjectionType', 0) == 1 else 'PERSP'

    camera.lens = elem_props_get_number(fbx_props, b'FocalLength', 35.0)
    camera.sensor_width = elem_props_get_number(
        fbx_props, b'FilmWidth', 32.0 * M2I) / M2I
    camera.sensor_height = elem_props_get_number(
        fbx_props, b'FilmHeight', 32.0 * M2I) / M2I

    camera.ortho_scale = elem_props_get_number(fbx_props, b'OrthoZoom', 1.0)

    filmaspect = camera.sensor_width / camera.sensor_height
    # film offset
    camera.shift_x = elem_props_get_number(
        fbx_props, b'FilmOffsetX', 0.0) / (M2I * camera.sensor_width)
    camera.shift_y = elem_props_get_number(
        fbx_props, b'FilmOffsetY', 0.0) / (M2I * camera.sensor_height * filmaspect)

    camera.clip_start = elem_props_get_number(
        fbx_props, b'NearPlane', 0.01) * global_scale
    camera.clip_end = elem_props_get_number(
        fbx_props, b'FarPlane', 100.0) * global_scale
...
def blen_read_light(fbx_tmpl, fbx_obj, global_scale):
    ...
    lamp.shadow_color = elem_props_get_color_rgb(
...
class FbxImportHelperNode:
    ...
    __slots__ = (
        '_parent', 'anim_compensation_matrix', 'is_global_animation', 'armature_setup', 'armature',
        'bind_matrix', 'bl_bone', 'bl_data', 'bl_obj', 'bone_child_matrix', 'children', 'clusters',
        'fbx_elem', 'fbx_name', 'fbx_transform_data', 'fbx_type', 'is_armature',
        'has_bone_children', 'is_bone', 'is_root', 'is_leaf', 'matrix', 'matrix_as_parent',
        'matrix_geom', 'meshes', 'post_matrix', 'pre_matrix')

    def __init__(self, fbx_elem, bl_data, fbx_transform_data, is_bone):
        self.fbx_name = elem_name_ensure_class(
            fbx_elem, b'Model') if fbx_elem else 'Unknown'
        ...
        # For bones only, relevant armature node.
        self.armature = None
        ...
        self.is_leaf = False
        # correction matrix that needs to be applied before the FBX transform
        self.pre_matrix = None
        # for bones this is the matrix used to bind to the skin
        self.bind_matrix = None
        if fbx_transform_data:
            self.matrix, self.matrix_as_parent, self.matrix_geom = blen_read_object_transform_do(
                fbx_transform_data)
        else:
            self.matrix, self.matrix_as_parent, self.matrix_geom = (
                None, None, None)
        # correction matrix that needs to be applied after the FBX transform
        ...
        # List of meshes influenced by this bone.
        self.meshes = None
    ...
    def find_correction_matrix(self, settings, parent_correction_inv=None):
        ...
        if self.is_bone:
            if settings.automatic_bone_orientation:
                ...
                bone_children = tuple(
                    child for child in self.children if child.is_bone)
                if len(bone_children) == 0:
                    ...
                    if len(bone_children) == 1:
                        vec = bone_children[0].get_bind_matrix(
                        ).to_translation()
                        ...
                        if abs(vec[0]) > abs(vec[1]):
                            ...
                            best_axis = Vector(
                                (0, 1 if vec[1] >= 0 else -1, 0))
                    else:
                        # get the child directions once because they may be checked several times
                        child_locs = (child.get_bind_matrix().to_translation()
                                      for child in bone_children)
                        child_locs = tuple(loc.normalized()
                                           for loc in child_locs if loc.magnitude > 0.0)
                        ...
                        if False:
                            ...
                            for i in range(6):
                                ...
                                s = -1 if i % 2 == 1 else 1
                                test_axis = Vector(
                                    (s if a == 0 else 0, s if a == 1 else 0, s if a == 2 else 0))

                                # find max angle to children
                                max_angle = 1.0
                                for loc in child_locs:
                                    max_angle = min(
                                        max_angle, test_axis.dot(loc))
    ...
    def find_fake_bones(self, in_armature=False):
        if in_armature and not self.is_bone and self.has_bone_children:
            ...
            if self.fbx_type not in {b'Null', b'Root'}:
                node = FbxImportHelperNode(
                    self.fbx_elem, self.bl_data, None, False)
                self.fbx_elem = None
                self.bl_data = None
    ...
    def build_skeleton(self, arm, parent_matrix, parent_bone_size=1, force_connect_children=False):
        def child_connect(par_bone, child_bone, child_head, connect_ctx):
            ...
            if similar_values_iter(par_bone.tail, child_head):
                ...
                if connected is ...:
                    connected = ([child_head.copy(), 1], [child_bone]
                                 if child_bone is not None else [])
                else:
        ...
        for child in self.children:
            if child.is_leaf and force_connect_children:
                ...
                child_head = (
                    bone_matrix @ child.get_bind_matrix().normalized()).translation
    ...
    def build_node_obj(self, fbx_tmpl, settings):
        ...
        obj.color[0:3] = elem_props_get_color_rgb(
            fbx_props, b'Color', (0.8, 0.8, 0.8))
    ...
    def link_skeleton_children(self, fbx_tmpl, settings, scene):
        if self.is_bone:
            ...
            for child in self.children:
                if child.ignore:
                    continue
                child_obj = child.link_skeleton_children(
                    fbx_tmpl, settings, scene)
                if child_obj:
    ...
    def merge_weights(self, combined_weights, fbx_cluster):
        indices = elem_prop_first(elem_find_first(
            fbx_cluster, b'Indexes', default=None), default=())
        weights = elem_prop_first(elem_find_first(
            fbx_cluster, b'Weights', default=None), default=())
    ...
    def set_bone_weights(self):
        ...
        if len(ignored_children) > 0:
            ...
            for fbx_cluster, meshes in self.clusters:
                ...
                add_vgroup_to_objects(indices, weights, self.bl_bone, [
                                      node.bl_obj for node in meshes])
            ...
            for child in ignored_children:
                for child_cluster, child_meshes in child.clusters:
                    if all_meshes.isdisjoint(child_meshes):
                        indices = elem_prop_first(elem_find_first(
                            child_cluster, b'Indexes', default=None), default=())
                        weights = elem_prop_first(elem_find_first(
                            child_cluster, b'Weights', default=None), default=())
                        add_vgroup_to_objects(indices, weights, self.bl_bone, [
    ...
    def build_hierarchy(self, fbx_tmpl, settings, scene, view_layer):
        if self.is_armature:
            ...
            self.bl_data = arm_data = bpy.data.armatures.new(
                name=elem_name_utf8)
            ...
            # Can't switch to Edit mode hidden objects...
            ...
            for child in self.children:
                ...
                if child.is_bone:
                    child.build_skeleton(
                        self, Matrix(), force_connect_children=settings.force_connect_children)
        ...
        if self.is_armature:
            ...
            if self.meshes:
                for mesh in self.meshes:
                    ...
                    amat = settings.global_matrix @ (Matrix()
                                                     if amat is None else amat)
                    if self.matrix_geom:
                    ...
                    me_obj.matrix_parent_inverse = amat.inverted_safe(
                    ) @ mmat @ me_obj.matrix_basis.inverted_safe()
...
def find_bsfbx(doc_node):
    # used for loading BGS materials from an FBX document node

    def check_elem(e):
        # recursive document search for the BSFBX document
        bsfbx = None
        if e.id == b'P' and e.props and e.props[0] == b'BSFBX':
            return e.props[4]
        else:
            if e.elems:
                for oe in e.elems:
                    bsfbx = check_elem(oe)
                    if bsfbx is not None:
                        break
        return bsfbx

    bsfbx = check_elem(doc_node)

    if bsfbx is not None:
        import json
        bsfbx = json.loads(bsfbx)
...
def load(operator, context, filepath="",
    ...
    if is_ascii:
        operator.report(
            {'ERROR'}, "ASCII FBX files are not supported %r" % filepath)
        return {'CANCELLED'}
    del is_ascii
    # End ascii detection.
    ...
    try:
        ...
        operator.report({'ERROR'}, "Couldn't open file %r (%s)" %
                        (filepath, e))
        return {'CANCELLED'}

    if version < 7100:
        operator.report(
            {'ERROR'}, "Version %r unsupported, must be %r or later" % (version, 7100))
        return {'CANCELLED'}
    ...
    if fbx_settings is None or fbx_settings_props is None:
        operator.report(
            {'ERROR'}, "No 'GlobalSettings' found in file %r" % filepath)
        return {'CANCELLED'}

    # FBX default base unit seems to be the centimeter, while raw Blender Unit is equivalent to the meter...
    unit_scale = elem_props_get_number(
        fbx_settings_props, b'UnitScaleFactor', 1.0)
    unit_scale_org = elem_props_get_number(
        fbx_settings_props, b'OriginalUnitScaleFactor', 1.0)
    ...
    if not use_manual_orientation:
        ...
        axis_up, axis_forward = {
            v: k for k, v in RIGHT_HAND_AXES.items()}.get(axis_key, ('Z', 'Y'))
    ...
    custom_fps = elem_props_get_number(
        fbx_settings_props, b'CustomFrameRate', 25.0)
    time_mode = elem_props_get_enum(fbx_settings_props, b'TimeMode')
    real_fps = {eid: val for val, eid in FBX_FRAMERATES[1:]}.get(
        time_mode, custom_fps)
    if real_fps <= 0.0:
        real_fps = 25.0
    ...
    settings = FBXImportSettings(operator.report, (axis_up, axis_forward),
                                 global_matrix, global_scale, bake_space_transform,
                                 global_matrix_inv, global_matrix_inv_transposed,
                                 use_custom_normals, use_image_search, use_bgs_materials,
                                 use_alpha_decals, decal_offset, use_anim, anim_offset,
                                 use_subsurf, use_custom_props, use_custom_props_enum_as_string,
                                 nodal_material_wrap_map, image_cache, ignore_leaf_bones,
    ...
    # BEGIN BGS STUFF ================================================================================================================================================================
    # look at the existing materials we have, and create a dictionary of mats that already have BSFBX associations- that way we don't load the same material twice.
    # look at the incoming materials in the BSFBX dictionary, create logical connections between the ones in bpy.data and this incoming FBX
    # create any new materials that were not already covered by existing materials, and make sure to set the MAT_PATH property and name them correctly so they can be loaded properly.
    # find the bsfbx document, and if it exists, convert it to JSON
    # fbx_docs = elem_find_first(elem_root, b'Documents')
    # bsfbx = find_bsfbx(fbx_docs)
    # END BGS STUFF
    ...
    if fbx_connections is None:
        operator.report(
            {'ERROR'}, "No 'Connections' found in file %r" % filepath)
        return {'CANCELLED'}
    ...
    def _():
        for fbx_link in fbx_connections.elems:
            ...
            if fbx_link.props_type[1:3] == b'LL':
                ...
                fbx_connection_map.setdefault(
                    c_src, []).append((c_dst, fbx_link))
                fbx_connection_map_reverse.setdefault(
                    c_dst, []).append((c_src, fbx_link))
    _()
    del _
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.id != b'Video':
                continue
            fbx_item[1] = blen_read_texture_image(
                fbx_tmpl_img, fbx_obj, basedir, settings)
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            fbx_obj, blen_data = fbx_item
            if fbx_obj.id != b'Texture':
                continue
            fbx_item[1] = blen_read_texture_image(
                fbx_tmpl_tex, fbx_obj, basedir, settings)
    _()
    del _
    ...
    def _():
        ...
        # create scene root
        fbx_helper_nodes[0] = root_helper = FbxImportHelperNode(
        ...
        for a_uuid, a_item in fbx_table_nodes.items():
            ...
            transform_data = blen_read_object_transform_preprocess(
                fbx_props, fbx_obj, Matrix(), use_prepost_rot)
            ...
            fbx_helper_nodes[a_uuid] = FbxImportHelperNode(
                fbx_obj, bl_data, transform_data, is_bone)
        ...
        for fbx_link in fbx_connections.elems:
            ...
            if fbx_link.props_type[1:3] == b'LL':
                ...
                if child is None:
                    ...
                    fbx_sdata, bl_data = p_item = fbx_table_nodes.get(
                        c_src, (None, None))
        ...
        for a_uuid, a_item in fbx_table_nodes.items():
            ...
            for fbx_pose_node in fbx_obj.elems:
                ...
                matrix = array_to_matrix4(
                    matrix_elem.props[0]) if matrix_elem else None
        ...
        for helper_uuid, helper_node in fbx_helper_nodes.items():
            ...
            for cluster_uuid, cluster_link in fbx_connection_map.get(helper_uuid, ()):
                if cluster_link.props[0] != b'OO':
                    continue
                fbx_cluster, _ = fbx_table_nodes.get(
                    cluster_uuid, (None, None))
                if fbx_cluster is None or fbx_cluster.id != b'Deformer' or fbx_cluster.props[2] != b'Cluster':
                    continue

                # Get the bind pose from the cluster:
                tx_mesh_elem = elem_find_first(
                    fbx_cluster, b'Transform', default=None)
                tx_mesh = array_to_matrix4(
                    tx_mesh_elem.props[0]) if tx_mesh_elem else Matrix()

                tx_bone_elem = elem_find_first(
                    fbx_cluster, b'TransformLink', default=None)
                tx_bone = array_to_matrix4(
                    tx_bone_elem.props[0]) if tx_bone_elem else None

                tx_arm_elem = elem_find_first(
                    fbx_cluster, b'TransformAssociateModel', default=None)
                tx_arm = array_to_matrix4(
                    tx_arm_elem.props[0]) if tx_arm_elem else None
                ...
                for skin_uuid, skin_link in fbx_connection_map.get(cluster_uuid):
                    ...
                    for mesh_uuid, mesh_link in fbx_connection_map.get(skin_uuid):
                        if mesh_link.props[0] != b'OO':
                            continue
                        fbx_mesh, _ = fbx_table_nodes.get(
                            mesh_uuid, (None, None))
                        ...
                        for object_uuid, object_link in fbx_connection_map.get(mesh_uuid):
                            ...
                            if mesh_node:
                                ...
                                mesh_node.armature_setup[helper_node.armature] = (
                                    mesh_matrix, armature_matrix)
    ...
    def _():
        ...
        for s_uuid, s_item in fbx_table_nodes.items():
            fbx_sdata, bl_sdata = s_item = fbx_table_nodes.get(
                s_uuid, (None, None))
            ...
            for bc_uuid, bc_ctype in fbx_connection_map.get(s_uuid, ()):
                if bc_ctype.props[0] != b'OO':
                    continue
                fbx_bcdata, _bl_bcdata = fbx_table_nodes.get(
                    bc_uuid, (None, None))
                ...
                for bs_uuid, bs_ctype in fbx_connection_map.get(bc_uuid, ()):
                    if bs_ctype.props[0] != b'OO':
                        continue
                    fbx_bsdata, _bl_bsdata = fbx_table_nodes.get(
                        bs_uuid, (None, None))
                    ...
                    for m_uuid, m_ctype in fbx_connection_map.get(bs_uuid, ()):
                        if m_ctype.props[0] != b'OO':
                            continue
                        fbx_mdata, bl_mdata = fbx_table_nodes.get(
                            m_uuid, (None, None))
                ...
                keyblocks = blen_read_shape(
                    fbx_tmpl, fbx_sdata, fbx_bcdata, meshes, scene)
        ...
        def _():
            for fbx_link in fbx_connections.elems:
                ...
                if fbx_link.props_type[1:3] == b'LL':
                    ...
                    child = fbx_helper_nodes.get(c_src)
                    if child is None:
                        fbx_sdata, bl_data = fbx_table_nodes.get(
                            c_src, (None, None))
                        if fbx_sdata.id != b'Geometry':
                            continue

                        preview_levels = elem_prop_first(
                            elem_find_first(fbx_sdata, b'PreviewDivisionLevels'))
                        render_levels = elem_prop_first(
                            elem_find_first(fbx_sdata, b'RenderDivisionLevels'))
                        if isinstance(preview_levels, int) and isinstance(render_levels, int):
                            mod = parent.bl_obj.modifiers.new(
                                'subsurf', 'SUBSURF')
    ...
    if use_anim:
        ...
        # Animation!
        def _():
            fbx_tmpl_astack = fbx_template_get(
                (b'AnimationStack', b'FbxAnimStack'))
            fbx_tmpl_alayer = fbx_template_get(
                (b'AnimationLayer', b'FbxAnimLayer'))
            stacks = {}
            ...
            def get_astacks_from_alayer(al_uuid):
                for as_uuid, as_ctype in fbx_connection_map.get(al_uuid, ()):
                    if as_ctype.props[0] != b'OO':
                        continue
                    fbx_asdata, _bl_asdata = fbx_table_nodes.get(
                        as_uuid, (None, None))
            ...
            for acn_uuid, fbx_acnitem in fbx_table_nodes.items():
                ...
                for n_uuid, n_ctype in fbx_connection_map.get(acn_uuid, ()):
                    ...
                    if lnk_prop in {b'Lcl Translation', b'Lcl Rotation', b'Lcl Scaling'}:
                        ...
                        print(
                            "WARNING! Importing material's animation is not supported for Nodal materials...")
                for al_uuid, al_ctype in fbx_connection_map.get(acn_uuid, ()):
                    if al_ctype.props[0] != b'OO':
                        continue
                    fbx_aldata, _blen_aldata = fbx_alitem = fbx_table_nodes.get(
                        al_uuid, (None, None))
                    ...
                    for as_uuid in get_astacks_from_alayer(al_uuid):
                        ...
                        for item, item_prop in items:
                            ...
                            anim_items.setdefault(item, {})[
                                acn_uuid] = (cnode, item_prop)
            ...
            for ac_uuid, fbx_acitem in fbx_table_nodes.items():
                ...
                for acn_uuid, acn_ctype in fbx_connection_map.get(ac_uuid, ()):
                    if acn_ctype.props[0] != b'OP':
                        continue
                    fbx_acndata, _bl_acndata = fbx_table_nodes.get(
                        acn_uuid, (None, None))
            ...
            blen_read_animations(
                fbx_tmpl_astack, fbx_tmpl_alayer, stacks, scene, settings.anim_offset)
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if mesh.validate_material_indices():
                print(
                    "WARNING: mesh '%s' had invalid material indices, those were reset to first material" % mesh.name)
    _()
    del _
    ...
    def _():
        ...
        def texture_mapping_set(fbx_obj, node_texture):
            ...
            fbx_props = (elem_find_first(fbx_obj, b'Properties70'),
                         elem_find_first(fbx_tmpl, b'Properties70', fbx_elem_nil))
            loc = elem_props_get_vector_3d(
                fbx_props, b'Translation', (0.0, 0.0, 0.0))
            rot = tuple(-r for r in elem_props_get_vector_3d(fbx_props,
                        b'Rotation', (0.0, 0.0, 0.0)))
            scale = tuple(((1.0 / s) if s != 0.0 else 1.0)
                          for s in elem_props_get_vector_3d(
                              fbx_props, b'Scaling', (1.0, 1.0, 1.0)))
            ...
            for (fbx_lnk,
                ...
                if fbx_lnk_type.props[0] == b'OP':
                    ...
                    if lnk_type in {b'DiffuseColor', b'3dsMax|maps|texmap_diffuse'}:
                        ...
                        texture_mapping_set(
                            fbx_lnk, ma_wrap.base_color_texture)
                    ...
                    # XXX, applications abuse bump!
                        ...
                        texture_mapping_set(
                            fbx_lnk, ma_wrap.emission_color_texture)
                    elif lnk_type in {b'EmissiveFactor'}:
                        ma_wrap.emission_strength_texture.image = image
                        texture_mapping_set(
                            fbx_lnk, ma_wrap.emission_strength_texture)
                    else:
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.id != b'Material':
                continue
            material = fbx_table_nodes.get(fbx_uuid, (None, None))[1]
            image = material_images.get(
                material, {}).get(b'DiffuseColor', None)
            # do we have alpha?

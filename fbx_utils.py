...
# This is the number of "ktimes" in one second (yep, precision over the nanosecond...)
FBX_KTIME = 46186158000


# Blender is -Z, FBX is -Y.
MAT_CONVERT_LIGHT = Matrix.Rotation(math.pi / 2.0, 4, 'X')
# Blender is -Z, FBX is +X.
        ...
        def level_down(self, message=""):
            ...
            print("\t" * self.level,
                  "\tDone (%f sec)\n" % ((time.process_time() - ref_time)
                                         if ref_time is not None else 0.0),
                  sep="")
            if message:
                print("\t" * self.level, message, sep="")
...
def units_blender_to_fbx_factor(scene):
    return 100.0 if (
        scene.unit_settings.system == 'NONE') else (
        100.0 * scene.unit_settings.scale_length)
    ...
    if uuid in uuids:
        ...
        while uuid in uuids:
            ...
            if 0 > uuid >= 2**63:
                ...
                raise ValueError(
                    "Unable to generate an UUID for key {}".format(key))
    return UUID(uuid)
...
def get_blender_anim_curve_node_key(scene, ref_id, obj_key, fbx_prop_name):
    ...
    return "|".join((get_blender_anim_id_base(scene, ref_id),
                     obj_key, fbx_prop_name, "AnimCurveNode"))
...
FBX_PROPERTIES_DEFINITIONS = {
    # Generic types.
    # Yes, int32 for a bool (and they do have a core bool type)!!!
    "p_bool": (b"bool", b"", "add_int32"),
    ...
    # Animatable-only?
    "p_vector": (b"Vector", b"", "add_float64", "add_float64", "add_float64"),
    # Non-animatable?
    "p_color_rgb": (b"ColorRGB", b"Color", "add_float64", "add_float64", "add_float64"),
    # Animatable-only?
    "p_color": (b"Color", b"", "add_float64", "add_float64", "add_float64"),
...
def elem_props_set(elem, ptype, name, value=None, animatable=False, animated=False, custom=False):
    ...
    _elem_props_set(elem, ptype, name, value,
                    _elem_props_flags(animatable, animated, custom))
...
def elem_props_compound(elem, cmpd_name, custom=False):
    def _setter(ptype, name, value, animatable=False, animated=False, custom=False):
        ...
        elem_props_set(elem, ptype, name, value,
                       animatable=animatable, animated=animated, custom=custom)
...
def elem_props_template_init(templates, template_type):
    ...
    if tmpl is not None:
        ...
        ret = {name: [val, ptype, anim, written]
               for name, (val, ptype, anim) in props.items()}
    return ret


def elem_props_template_set(
    ...
    if len(ptype) > 3:
        value = tuple(value)
    tmpl_val, tmpl_ptype, tmpl_animatable, tmpl_written = template.get(
    ...
    if tmpl_ptype is not None and not animated:
        ...
        _elem_props_set(elem, ptype, name, value, _elem_props_flags(
            tmpl_animatable, animated, False))
        template[name][3] = True
    else:
        _elem_props_set(elem, ptype, name, value,
                        _elem_props_flags(animatable, animated, False))
...
def elem_props_template_finalize(template, elem):
    ...
    for name, (value, ptype_name, animatable, written) in template.items():
        ...
        ptype = FBX_PROPERTIES_DEFINITIONS[ptype_name]
        _elem_props_set(elem, ptype, name, value,
                        _elem_props_flags(animatable, False, False))


# ##### Templates #####
# TODO: check all those "default" values, they should match Blender's default as much as possible, I guess?

FBXTemplate = namedtuple("FBXTemplate", ("type_name",
                         "prop_type_name", "properties", "nbr_users", "written"))
...
def fbx_templates_generate(root, fbx_templates):
    ...
    for type_name, (subprops, nbr_users) in templates.items():
        ...
        if len(subprops) == 1:
            prop_type_name, (properties, _nbr_sub_type_users) = next(
                iter(subprops.items()))
            ...
            for prop_type_name, (properties, nbr_sub_type_users) in subprops.items():
                if nbr_sub_type_users > max_users or (
                        nbr_sub_type_users == max_users and len(properties) > max_props):
            ...
            ref_templates[(type_name, written_prop_type_name)
                          ].written[0] = True

        prop_type_name, properties = subprops
        if prop_type_name and properties:
            elem = elem_data_single_string(
                template, b"PropertyTemplate", prop_type_name)
...
class AnimationCurveNodeWrapper:
    ...
    def add_keyframe(self, frame, values):
        ...
        self._keys.append((frame, values, [True] * len(values)))
    ...
    def simplify(self, fac, step, force_keep=False):
        ...
        # min relative value evolution: 0.1% of current 'order of magnitude'.
        min_reldiff_fac = fac * 1.0e-3
    ...
    def get_final_data(self, scene, ref_id, force_keep=False):
        ...
        for elem_key, fbx_group, fbx_gname, fbx_props in \
            ...
            group_key = get_blender_anim_curve_node_key(
                scene, ref_id, elem_key, fbx_group)
            group = {}
            for c, def_val, fbx_item in zip(curves, self.default_values, fbx_props):
                fbx_item = FBX_ANIM_PROPSGROUP_NAME + "|" + fbx_item
                curve_key = get_blender_anim_curve_key(
    ...
    def __call__(cls, bdata, armature=None):
        ...
        if isinstance(bdata, Object):
            ...
            if bdata.is_instance:
                key = "|".join((
                    get_blenderID_key(
                        (bdata.parent.original, bdata.instance_object.original)),
    ...
    * Blender DepsgraphObjectInstance (for dulis).
    ...
    def get_parent(self):
        if self._tag == 'OB':
            if (self.bdata.parent and self.bdata.parent.type == 'ARMATURE' and
                ...
                bo_par = self.bdata.parent.pose.bones.get(
                    self.bdata.parent_bone, None)
                if (bo_par):
    ...
    def get_matrix_local(self):
        if self._tag == 'OB':
            ...
            par_mat_inv = self._ref.pose.bones[par.name].matrix.inverted_safe(
            ) if par else Matrix()
        ...
        if par in objects:
            if self._tag == 'OB':
                ...
                if par_type in {'OBJECT', 'BONE'}:
                    ...
                    print(
                        "Sorry, “{}” parenting type is not supported".format(par_type))
                    return False
            return True
        return False
    ...
    def fbx_object_matrix(self, scene_data, rest=False, local_space=False, global_space=False):
        ...
        is_global = (not local_space and (global_space or not (
            self._tag in {'DP', 'BO'} or self.has_valid_parent(scene_data.objects))))
        ...
        if bsutil.DO_PARENT_ARMATURE_CHILD != True:
            # this behaviour in the default fbx exported uses the global transform for children of armatures (should be disabled)
        ...
        if self._tag in {'DP', 'OB'} and parent:
            if parent._tag == 'BO':
                ...
                matrix = Matrix.Translation(
                    (0, (parent.bdata.tail - parent.bdata.head).length, 0)) @ matrix
        ...
        if parent:
            if is_global:
                ...
                matrix = (
                    parent.matrix_rest_global if rest else parent.matrix_global) @ matrix
            elif parent.use_bake_space_transform(scene_data):
                # Blender's and FBX's local space of parent may differ if we use bake_space_transform...
                # Apply parent's *Blender* local space...
                matrix = (
                    parent.matrix_rest_local if rest else parent.matrix_local) @ matrix
                # ...and move it back into parent's *FBX* local space.
                par_mat = parent.fbx_object_matrix(
                    scene_data, rest=rest, local_space=True)
    ...
    def is_deformed_by_armature(self, arm_obj):
        ...
        for mod in self.bdata.modifiers:
            # remove: arm_obj.bdata.proxy
            if mod.type == 'ARMATURE' and mod.object in {arm_obj.bdata}:
                return True
...
FBXExportSettings = namedtuple("FBXExportSettings",
                               ("report", "to_axes", "global_matrix", "global_scale",
                                "apply_unit_scale", "unit_scale", "bake_space_transform",
                                "global_matrix_inv", "global_matrix_inv_transposed",
                                "context_objects", "object_types", "use_mesh_modifiers",
                                "use_mesh_modifiers_render", "mesh_smooth_type", "use_subsurf",
                                "use_mesh_edges", "use_tspace", "use_bgs_materials",
                                "armature_nodetype", "use_armature_deform_only", "add_leaf_bones",
                                "bone_correction_matrix", "bone_correction_matrix_inv",
                                "bake_anim", "bake_anim_use_all_bones",
                                "bake_anim_use_nla_strips", "bake_anim_use_all_actions",
                                "bake_anim_step", "bake_anim_simplify_factor",
                                "bake_anim_force_startend_keying", "use_metadata",
                                "media_settings", "use_custom_props",))
...
FBXExportData = namedtuple("FBXExportData",
                           ("templates", "templates_users", "connections", "settings", "scene",
                            "depsgraph", "objects", "animations", "animated", "frame_start",
                            "frame_end", "data_empties", "data_lights", "data_cameras",
                            "data_meshes", "mesh_material_indices", "data_bones",
                            "data_leaf_bones", "data_deformers_skin", "data_deformers_shape",
                            "data_world", "data_materials", "data_textures", "data_videos",))

# Helper container gathering all importer settings.
FBXImportSettings = namedtuple("FBXImportSettings",
                               ("report", "to_axes", "global_matrix", "global_scale",
                                "bake_space_transform", "global_matrix_inv",
                                "global_matrix_inv_transposed", "use_custom_normals",
                                "use_image_search", "use_bgs_materials", "use_alpha_decals",
                                "decal_offset", "use_anim", "anim_offset", "use_subsurf",
